...
class AdminBusinessServiceTests {
    ...
    // // Test when residentId is null
    // @Test
    // void testGetResident_NullResidentId() throws Exception {
    // // Arrange: Use null residentId and mock the dataService to return a default
    // resident
    // when(dataService.findResident(null)).thenReturn(defaultResident);

    // // Act: Invoke the method with null residentId
    // AdminResidentModel result = adminBusinessService.getResident(null);

    // // Assert: Verify the result (assuming dataService handles null gracefully)
    // assertNotNull(result, "Result should not be null");
    // assertEquals(defaultResident.getBuildingId(), result.getBuildingId(),
    // "Building ID should match");
    // assertEquals(defaultResident.getResidentId(), result.getResidentId(),
    // "Resident ID should match");
    // assertEquals(defaultResident.getFirstName(), result.getFirstName(), "First
    // name should match");
    // assertEquals(defaultResident.getLastName(), result.getLastName(), "Last name
    // should match");
    // assertEquals(defaultResident.getEmail(), result.getEmail(), "Email should
    // match");
    // assertEquals(defaultResident.getFloor(), result.getFloor(), "Floor should
    // match");
    // assertEquals(defaultResident.getRoom(), result.getRoom(), "Room should
    // match");
    // assertEquals(defaultResident.getLeaseStart(), result.getStartDate(), "Lease
    // start should match");
    // assertEquals(defaultResident.getLeaseEnd(), result.getEndDate(), "Lease end
    // should match");
    // verify(dataService, times(1)).findResident(null);
    // verifyNoMoreInteractions(dataService);
    // }

    // // Test when dataService throws a different exception (e.g., database error)
    // @Test
    // void testGetResident_DatabaseError() throws Exception {
    // // Arrange: Mock the dataService to throw a database error for the global
    // residentId
    // Exception expectedException = new Exception("Database connection failed");
    // when(dataService.findResident(residentId)).thenThrow(expectedException);
    ...
    void testFindResidents_SuccessfulRetrieval_FloorDescSort() throws Exception {
        ...
        assertEquals(2, result.size(), "Expected two residents in the result");
        assertTrue(result.stream().anyMatch(r -> r.getFirstName().equals("John")), "John should be in the result");
        assertTrue(result.stream().anyMatch(r -> r.getFirstName().equals("John")), "John should be in the result");
    ...
    // TODO - won't controller catch if it's null
    // // Test when buildingId is null
    // @Test
    // void testFindResidents_NullBuildingId() throws Exception {
    // // Arrange: Use null buildingId and mock the dataService to return
    // residentEntities
    // when(dataService.findResidents(null, true,
    // pageable)).thenReturn(residentEntities);

    // // Act: Invoke the method with null buildingId
    // List<AdminResidentModel> result = adminBusinessService.getResidents(null,
    // true, pageable);

    // // Assert: Verify the result (assuming dataService handles null gracefully)
    // assertEquals(2, result.size(), "Expected two residents in the result");
    // assertTrue(result.stream().anyMatch(r -> r.getFirstName().equals("Alice")),
    // "Alice should be in the result");
    // assertTrue(result.stream().anyMatch(r -> r.getFirstName().equals("Bob")),
    // "Bob should be in the result");
    // verify(dataService, times(1)).findResidents(null, true, pageable);
    // verifyNoMoreInteractions(dataService);
    // }
